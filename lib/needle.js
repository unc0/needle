...
var version     = JSON.parse(fs.readFileSync(__dirname + '/../package.json').toString()).version,
    debugging   = /^(needle|[*])/.test(process.env.DEBUG),
...
var Needle = {
  ...
  request: function(method, uri, data, options, callback) {
    ...
    var config = {
      ...
      retries         : options.retries === true ? 3 : typeof options.retries == 'number' ? options.retries : defaults.retries,
  ...
  send_request: function(count, method, uri, config, post_data, out, callback) {
    ...
    request.on('error', function(err) {
      var e = err || new Error('Unknown error when making request.');
      debug('Request error', e);
      if (timer) timer.clear();
      if (count <= config.retries) {
        out.emit('retry', count);
        debug('Retrying', count, uri);
        return self.send_request(++count, 'GET', uri, config, null, out, callback);
      }
      if (config.retries > 0)
        e = new Error('Max retries reached.');
      return done(e);
    });
