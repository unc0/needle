...
>>>>>>> Added follow_options, to allow further customization of the behavior when getting a 301/302/303 response containing a Location header.
}
...
var Needle = {
  ...
  request: function(method, uri, data, options, callback) {
    ...
    if (options.follow && (options.follow !== false || options.follow !== 0)) { // follow enabled
      var follow_opts = typeof options.follow == 'number' ? { max: options.follow } : options.follow;

      Object.keys(defaults.follow).forEach(function(key) {
        var config_key = 'follow_' + key;
        config[config_key] = follow_opts[key] || defaults.follow[key];
      })
    }
  ...
  should_follow: function(location, config, original) {
    ...
    var dest = url.parse(location);
    if (dest.host == original.host && dest.path == original.path && dest.protocol == original.protocol)
  ...
  send_request: function(count, method, uri, config, post_data, out, callback) {
    ...
    var request = protocol.request(request_opts, function(resp) {
      ...
      if ([301, 302, 303].indexOf(resp.statusCode) !== -1 && self.should_follow(headers.location, config, request_opts)) {
